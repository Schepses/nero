...
const char * const BITCOIN_CONF_FILENAME = "nero.conf";
const char * const BITCOIN_PID_FILENAME = "nerod.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "nero" is a composite category enabling all Nero-related debug output
            if(ptrCategory->count(string("nero"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NeroCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NeroCore
    // Mac: ~/Library/Application Support/NeroCore
    // Unix: ~/.nerocore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NeroCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NeroCore";
#else
    // Unix
    return pathRet / ".nerocore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty nero.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override nero.conf
