...
#if defined(NDEBUG)
# error "Nero Core cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Nero Signed Message:\n";
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight == 1) {
        ...
        return 1 * COIN;
    } else if (nPrevHeight > 1000 && nPrevHeight <= 10000) {
        return 20 * COIN;
    } else if (nPrevHeight > 10000 && nPrevHeight <= 20000) {
        return 22 * COIN;
    } else if (nPrevHeight > 20000 && nPrevHeight <= 40000) {
        return 24 * COIN;
    } else if (nPrevHeight > 40000 && nPrevHeight <= 70000) {
        return 26 * COIN;
    } else if (nPrevHeight > 70000 && nPrevHeight <= 100000) {
        return 24 * COIN;
    } else {
        return 20 * COIN;
    }
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // NERO : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(NERO): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(NERO): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // NERO : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(NERO): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(NERO): spork is off, skipping transaction locking checks\n");
    }
